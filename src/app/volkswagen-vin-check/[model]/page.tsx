...
import VolkswagenVinBody from "@/components/VolkswagenVinBody";
import {
  VOLKSWAGEN_MODELS,
  findVolkswagenModel,
  volkswagenFaqs,
...
export function generateStaticParams() {
  return VOLKSWAGEN_MODELS.map((m) => ({ model: m.slug }));
}
...
export async function generateMetadata({
  ...
  const m = findVolkswagenModel(model);
  if (!m) return {};
  ...
  return {
    ...
    alternates: { canonical: `/volkswagen-vin-check/${m.slug}` },
    openGraph: {
      title,
      description,
...
export default async function VolkswagenModelVinPage({
  params,
}: {
  params: Promise<{ model: string }>;
}) {
  ...
  const pageUrl = `${SITE}/volkswagen-vin-check/${m.slug}`;
  const faqs = volkswagenFaqs(m);
  const howTo = volkswagenHowTo(m);
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Volkswagen VIN Check", item: `${SITE}/volkswagen-vin-check` },
  ...
  return (
    <>
      ...
      <VolkswagenVinBody modelSlug={m.slug} />
    </>
  );
}
