...
const description = "Vérifie marques de titre de récupération, reconstruido, inondation, chatarra et citron par VIN — gratuit. Cruza NMVTIS et les 50 enregistrements de DMV d état pour mostrar titrois marques que le lavado de titre intenta ocultar, avant de acheter.";

export const metadata: Metadata = {
  title, description,
  keywords: ["vérification titre récupération", "VIN titre récupération", "vérification titre reconstruit", "vérification titre marque", "vérification titre inondation VIN", "recherche titre chatarra", "recherche VIN titre récupération gratuit", "vérifier titre récupération par VIN", "vérification récupération NMVTIS", "vérification lavado titre", "el titre est limpio", "titre reconstituido VIN"],
  alternates: { canonical: alt.canonical, languages: alt.languages },
  openGraph: { title, description, url: PAGE_URL, type: "article", siteName: "CarCheckerVIN", locale: "fr_US" },
  twitter: { card: "summary_large_image", title, description: "Recherche gratuite de récupération et titre marque basada en VIN à travers NMVTIS et les 50 enregistrements DMV d état. Derrota le lavado de titre." },
  robots: { index: true, follow: true },
};

const webAppSchema = { "@context": "https://schema.org", "@type": "WebApplication", inLanguage: "fr", name: "Vérification de titre de récupération par VIN", url: PAGE_URL, applicationCategory: "AutomotiveApplication", operatingSystem: "All", description: "Vérifie le historique de marque de titre de un véhicule par su VIN de 17 caractères. Cruza NMVTIS, archivos de marque de titre de DMV d état et feeds de perte totale de assureurs pour mostrar marques de récupération, reconstruido, inondation, chatarra et citron — incluso quand le titre físico ha été lavado limpio.", offers: { "@type": "Offer", price: "0", priceCurrency: "USD" }, publisher: { "@type": "Organization", name: "CarCheckerVIN", url: SITE, logo: { "@type": "ImageObject", url: `${SITE}/logo.png` } } };
const articleSchema = { "@context": "https://schema.org", "@type": "Article", inLanguage: "fr", headline: "Vérification de titre de récupération par VIN", description: "Apprends que significan les titrois de récupération et marques, comment identificarlos, les risques réels de acheter unon et comment una vérification VIN derrota le lavado de titre.", author: ORG_AUTHOR, publisher: { "@type": "Organization", name: "CarCheckerVIN", url: SITE, logo: { "@type": "ImageObject", url: `${SITE}/logo.png` } }, mainEntityOfPage: { "@type": "WebPage", "@id": PAGE_URL }, datePublished: "2026-04-16", dateModified: new Date().toISOString().slice(0, 10) };
