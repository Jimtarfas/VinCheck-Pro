/**
 * Wave 17g — French blog category archive.
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const description =
    ...
    `Lis les artículos plus recientes de CarCheckerVIN en la categoría ${category.title} — guides, consejos et analyse.`;
...
export default async function Page({ params }: Props) {
  ...
  const pageUrl = `${SITE}/fr/blog/category/${category.slug}`;
  ...
  return (
    <>
      ...
      />
      <BlogCategoryBody category={category} posts={posts} locale="fr" />
    </>
  );
}
