...
import FordVinBody from "@/components/FordVinBody";
import {
  FORD_MODELS,
  findFordModel,
  fordFaqs,
...
export function generateStaticParams() {
  return FORD_MODELS.map((m) => ({ model: m.slug }));
}
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `/ford-vin-check/${m.slug}` },
    openGraph: {
      title,
      description,
...
export default async function FordModelVinPage({
  params,
}: {
  params: Promise<{ model: string }>;
}) {
  ...
  const pageUrl = `${SITE}/ford-vin-check/${m.slug}`;
  const faqs = fordFaqs(m);
  const howTo = fordHowTo(m);
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Ford VIN Check", item: `${SITE}/ford-vin-check` },
