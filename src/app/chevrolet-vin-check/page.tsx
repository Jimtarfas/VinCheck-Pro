...
const TITLE = "Chevrolet VIN Check & Decoder — Free by Model";
const DESCRIPTION =
  "Free Chevrolet VIN check. Decode any Chevy by model — Silverado, Equinox, Tahoe, Corvette and more — and reveal salvage, flood, buyback & accident brands. NMVTIS-backed, instant, no signup.";
...
export default function ChevroletVinHubPage() {
  const pageUrl = `${SITE}/chevrolet-vin-check`;
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Chevrolet VIN Check", item: pageUrl },
    ],
  };
  ...
  const itemListSchema = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    name: "Chevrolet VIN Check by Model",
    description: "Model-specific VIN check and decoder guides for popular Chevrolet vehicles.",
    itemListElement: CHEVY_MODELS.map((m, i) => ({
      "@type": "ListItem",
      position: i + 1,
      name: `${m.fullName} VIN Check`,
      url: `${SITE}/chevrolet-vin-check/${m.slug}`,
    })),
  };
        ...
        <div className="bg-primary text-white">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-24 pb-14 sm:pt-28 sm:pb-20">
            <Breadcrumbs
              items={[
                { label: "Home", href: "/" },
                { label: "Chevrolet VIN Check" },
              ]}
              onDark
            />

            <div className="mt-6 inline-flex items-center gap-2 bg-white/10 border border-white/20 rounded-full px-4 py-1.5 text-sm font-bold mb-4">
              <Fingerprint className="w-4 h-4" /> Chevrolet VIN Decoder &amp; History
            </div>

            <h1 className="text-3xl sm:text-5xl lg:text-6xl font-headline font-extrabold leading-tight mb-4">
              Chevrolet VIN Check —{" "}
              <span style={{ color: "var(--color-secondary-container)" }}>
                Decode Any Chevy by Model
              </span>
            </h1>

            <p className="speakable-intro text-base sm:text-xl text-white/85 max-w-3xl mb-8 leading-relaxed">
              Run a free Chevrolet VIN check to decode the year, engine, and trim
              of any Chevy, and reveal salvage, flood, lemon-law buyback,
            ...
            <div className="bg-white rounded-2xl p-5 sm:p-7 shadow-xl">
              <h2 className="text-base sm:text-lg font-headline font-extrabold text-primary mb-1">
                Run a Free Chevrolet VIN Check
              </h2>
              <p className="text-xs sm:text-sm text-on-surface-variant mb-4">
                Enter any 17-character Chevrolet VIN
              </p>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          <section className="py-12 sm:py-16 border-b border-outline-variant">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {CHEVY_MODELS.map((m) => (
                <Link
                  key={m.slug}
                  href={`/chevrolet-vin-check/${m.slug}`}
          ...
          <section className="py-12 sm:py-16 border-b border-outline-variant">
            ...
            <div className="prose prose-slate max-w-none text-sm sm:text-base text-on-surface-variant leading-relaxed space-y-4">
              <p>
                ...
                tell the two apart for the specific Chevy you are considering.
              </p>
              <p>
                The first three characters of any Chevrolet VIN are the GM World
                Manufacturer Identifier (WMI), assigned by brand, plant, and
                country — a US-built Chevy typically opens with{" "}
                <strong className="text-on-surface">1G</strong>, while Canada- and
                Mexico-built models open with <strong className="text-on-surface">2G</strong>{" "}
                or <strong className="text-on-surface">3G</strong>. Confirming the
                WMI against the title paperwork is a simple, effective re-VIN check.
              </p>
              <p>
          ...
          <section className="py-10">
            <div className="rounded-3xl bg-primary p-7 sm:p-10 text-center">
              ...
              <h2 className="text-2xl sm:text-3xl font-headline font-extrabold text-white mb-2">
                Check Any Chevrolet in Seconds
              </h2>
              <p className="text-white/80 text-sm sm:text-base mb-6 max-w-xl mx-auto">
                Free, instant Chevrolet VIN check sourced from NMVTIS and every
          ...
          <section className="py-14 sm:py-20 text-center">
            <div className="inline-flex items-center gap-2 bg-primary/10 text-primary rounded-full px-4 py-1.5 text-xs font-black uppercase tracking-wider mb-4">
              <Zap className="w-3.5 h-3.5" /> Free · Instant · Chevrolet
            </div>
            <h2 className="text-2xl sm:text-4xl font-headline font-extrabold text-primary mb-3">
              One VIN. Your Chevy&apos;s Full History. Five Seconds.
            </h2>
