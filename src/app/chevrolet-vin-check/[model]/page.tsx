...
import ChevyVinBody from "@/components/ChevyVinBody";
import {
  CHEVY_MODELS,
  findChevyModel,
  chevyFaqs,
...
export function generateStaticParams() {
  return CHEVY_MODELS.map((m) => ({ model: m.slug }));
}
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `/chevrolet-vin-check/${m.slug}` },
    openGraph: {
      title,
      description,
...
export default async function ChevyModelVinPage({
  params,
}: {
  params: Promise<{ model: string }>;
}) {
  ...
  const pageUrl = `${SITE}/chevrolet-vin-check/${m.slug}`;
  const faqs = chevyFaqs(m);
  const howTo = chevyHowTo(m);
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Chevrolet VIN Check", item: `${SITE}/chevrolet-vin-check` },
