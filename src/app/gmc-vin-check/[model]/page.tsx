...
import GmcVinBody from "@/components/GmcVinBody";
import {
  GMC_MODELS,
  findGmcModel,
  gmcFaqs,
...
export function generateStaticParams() {
  return GMC_MODELS.map((m) => ({ model: m.slug }));
}
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `/gmc-vin-check/${m.slug}` },
    openGraph: {
      title,
      description,
...
export default async function GmcModelVinPage({
  params,
}: {
  params: Promise<{ model: string }>;
}) {
  ...
  const pageUrl = `${SITE}/gmc-vin-check/${m.slug}`;
  const faqs = gmcFaqs(m);
  const howTo = gmcHowTo(m);
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "GMC VIN Check", item: `${SITE}/gmc-vin-check` },
