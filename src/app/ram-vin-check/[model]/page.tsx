...
import RamVinBody from "@/components/RamVinBody";
import {
  RAM_MODELS,
  findRamModel,
  ramFaqs,
...
export function generateStaticParams() {
  return RAM_MODELS.map((m) => ({ model: m.slug }));
}
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `/ram-vin-check/${m.slug}` },
    openGraph: {
      title,
      description,
...
export default async function RamModelVinPage({
  params,
}: {
  params: Promise<{ model: string }>;
}) {
  ...
  const pageUrl = `${SITE}/ram-vin-check/${m.slug}`;
  const faqs = ramFaqs(m);
  const howTo = ramHowTo(m);
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "RAM VIN Check", item: `${SITE}/ram-vin-check` },
