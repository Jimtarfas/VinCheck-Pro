...
const COPY = {
  ...
  fr: {
    ...
    makeHint: "Multiplicateur de rétention spécifique à la marque appliqué",
    vehicleType: "Type de véhicule",
    typeHint: "Le type de carrosserie influence la courbe de dépréciation",
    yearPurchased: "Année d'achat",
    yearHint: "Utilisée pour projeter les valeurs des années futures",
    purchasePrice: "Prix d'achat (USD)",
    priceHint: "Prix original / payé",
    annualMileage: "Kilométrage annuel (miles)",
    mileageHint: "12k/an est la référence du secteur",
    condition: "Hypothèse d'état",
    conditionHint: "À quel point le véhicule est bien entretenu dans le temps",
    calculate: "Calculer la dépréciation",
    reset: "Réinitialiser",
    headline: "Valeur projetée de ta voiture",
    after1: "Après 1 an",
    after3: "Après 3 ans",
    after5: "Après 5 ans",
    after10: "Après 10 ans",
    retainedShort: (pct: string) => `${pct}% retenu`,
    verdictGood: "Mieux que la moyenne du marché",
    verdictBad: "Plus rapide que la moyenne du marché",
    verdictNeutral: "Dépréciation proche de la moyenne",
    verdictGoodDesc: (make: string) =>
      `${make} conserve historiquement plus de valeur que le véhicule moyen — un choix financier solide à long terme.`,
    verdictBadDesc: (make: string) =>
      `${make} se déprécie généralement plus vite que le véhicule moyen. Pense à acheter d'occasion pour laisser le précédent propriétaire absorber la chute la plus forte.`,
    verdictNeutralDesc: (make: string) =>
      `${make} se déprécie près de la moyenne du secteur pour son segment.`,
    curveTitle: "Courbe de dépréciation sur 10 ans",
    curveFooter:
      "Années depuis l'achat \u00b7 Hauteur de barre = % du prix original retenu",
    tooltipYear: (age: number, value: string, pct: string) =>
      `Année ${age} : ${value} (${pct}%)`,
    summaryTotalLost: "Total $ perdu (10 ans)",
    summaryWorstYear: "Pire année",
    summaryWorstYearLabel: (age: number) => `Année ${age}`,
    summaryDropFmt: (pct: string) => `-${pct}% de chute`,
    summaryBrandMult: "Multiplicateur de marque",
    summaryAbove: "Au-dessus de la moyenne",
    summaryBelow: "En dessous de la moyenne",
    summaryAvg: "Moyenne",
    tableTitle: "Valeur projetée année par année",
    tableSpan: "10 ans",
    colCalendarYear: "Année civile",
    colAge: "Âge",
    colProjectedValue: "Valeur projetée",
    colLost: "$ perdu",
    colRetained: "% retenu",
    ageNew: "Neuf",
    ageYears: (age: number) => `${age} an${age !== 1 ? "s" : ""}`,
    disclaimer:
      "Les estimations utilisent des courbes de dépréciation composites du secteur avec des multiplicateurs de marque et de type de carrosserie. Les valeurs de revente réelles varient selon la finition, la région, le kilométrage et les conditions du marché. Pour la valeur marchande spécifique au VIN, lance un rapport d'historique de véhicule gratuit.",
    vehicleTypeLabels: {
      Sedan: "Berline",
      SUV: "SUV",
      Truck: "Pick-up",
