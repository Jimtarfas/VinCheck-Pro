...
import ToyotaVinBody from "@/components/ToyotaVinBody";
import {
  TOYOTA_MODELS,
  findToyotaModel,
  toyotaFaqs,
...
export function generateStaticParams() {
  return TOYOTA_MODELS.map((m) => ({ model: m.slug }));
}
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `/toyota-vin-check/${m.slug}` },
    openGraph: {
      title,
      description,
...
export default async function ToyotaModelVinPage({
  params,
}: {
  params: Promise<{ model: string }>;
}) {
  ...
  const pageUrl = `${SITE}/toyota-vin-check/${m.slug}`;
  const faqs = toyotaFaqs(m);
  const howTo = toyotaHowTo(m);
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Toyota VIN Check", item: `${SITE}/toyota-vin-check` },
  ...
  return (
    <>
      ...
      <ToyotaVinBody modelSlug={m.slug} />
    </>
  );
}
