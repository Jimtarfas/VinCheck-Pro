...
import DodgeVinBody from "@/components/DodgeVinBody";
import {
  DODGE_MODELS,
  findDodgeModel,
  dodgeFaqs,
...
export function generateStaticParams() {
  return DODGE_MODELS.map((m) => ({ model: m.slug }));
}
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `/dodge-vin-check/${m.slug}` },
    openGraph: {
      title,
      description,
...
export default async function DodgeModelVinPage({
  params,
}: {
  params: Promise<{ model: string }>;
}) {
  ...
  const pageUrl = `${SITE}/dodge-vin-check/${m.slug}`;
  const faqs = dodgeFaqs(m);
  const howTo = dodgeHowTo(m);
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Dodge VIN Check", item: `${SITE}/dodge-vin-check` },
