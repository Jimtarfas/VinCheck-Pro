/**
 * Shared metadata + JSON-LD generator for the Big-5 Spanish state pages.
 * Each /es/<state>-revision-vin/page.tsx imports `stateMetadata(hook)`
...
export function stateSchemas(hook: StateHook) {
  const articleSchema = {
    ...
    "@type": "Article",
    inLanguage: "es",
    headline: `Revisión VIN de ${hook.stateNameEs} — Reporte gratis del historial del vehículo (${hook.dmvNameEs})`,
    description: hook.metaDescription,
    author: ORG_AUTHOR,
    publisher: {
      "@type": "Organization",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "Inicio", item: `${SITE}/es` },
      {
        "@type": "ListItem",
        position: 2,
        name: `Revisión VIN de ${hook.stateNameEs}`,
