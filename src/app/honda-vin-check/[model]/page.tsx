...
import HondaVinBody from "@/components/HondaVinBody";
import {
  HONDA_MODELS,
  findHondaModel,
  hondaFaqs,
...
export function generateStaticParams() {
  return HONDA_MODELS.map((m) => ({ model: m.slug }));
}
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `/honda-vin-check/${m.slug}` },
    openGraph: {
      title,
      description,
...
export default async function HondaModelVinPage({
  params,
}: {
  params: Promise<{ model: string }>;
}) {
  ...
  const pageUrl = `${SITE}/honda-vin-check/${m.slug}`;
  const faqs = hondaFaqs(m);
  const howTo = hondaHowTo(m);
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Honda VIN Check", item: `${SITE}/honda-vin-check` },
