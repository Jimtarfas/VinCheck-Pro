...
import JeepVinBody from "@/components/JeepVinBody";
import {
  JEEP_MODELS,
  findJeepModel,
  jeepFaqs,
...
export function generateStaticParams() {
  return JEEP_MODELS.map((m) => ({ model: m.slug }));
}
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `/jeep-vin-check/${m.slug}` },
    openGraph: {
      title,
      description,
...
export default async function JeepModelVinPage({
  params,
}: {
  params: Promise<{ model: string }>;
}) {
  ...
  const pageUrl = `${SITE}/jeep-vin-check/${m.slug}`;
  const faqs = jeepFaqs(m);
  const howTo = jeepHowTo(m);
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Jeep VIN Check", item: `${SITE}/jeep-vin-check` },
