/**
 * NMVTIS disclaimer — bilingual.
 *
 * The English text is the federal canonical (49 U.S.C. § 30502 and
 * 28 C.F.R. Part 25, Subpart C). The Spanish rendering follows the
 * BJA/NMVTIS consumer-notice wording closely but it is a Wave-11
 * DRAFT pending sign-off by ClearVin LLC's compliance team. Until
 * sign-off lands, the Spanish view surfaces a banner pointing buyers
 * to the English canonical for full legal certainty.
 *
 * Locale is read from ?lang=es (server component, await searchParams).
 */
...
const DISCLAIMER_COPY: Record<Locale, DisclaimerCopy> = {
  en: {
    ...
    providerAttribution: {
      ...
      p2: "CarCheckerVIN is a reseller of ClearVin's NMVTIS data service. CarCheckerVIN does not modify, edit, alter, or omit any data value returned by ClearVin; only the surrounding layout and styling are customized for the CarCheckerVIN brand.",
    },
  ...
  es: {
    ...
    notice: {
      ...
      p2: "La ley federal exige que todos los estados, aseguradoras y desguazadores de chatarra y salvage reporten informaci\u00f3n al NMVTIS. Sin embargo, los datos NMVTIS los proveen los proveedores actuales, y los datos de cualquier veh\u00edculo pueden no estar en el sistema si los proveedores a\u00fan no est\u00e1n reportando. La informaci\u00f3n sobre la fecha de cualquier dato nuevo proporcionado se incluir\u00e1 en tu reporte.",
      ...
      p4Suffix:
        " que est\u00e1n obligadas por ley federal a reportar veh\u00edculos que han declarado p\u00e9rdida total. NMVTIS tambi\u00e9n recoge informaci\u00f3n de desguazadores de chatarra y salvage que est\u00e1n obligados por ley federal a reportar los veh\u00edculos que reciben en sus instalaciones.",
      p5Strong:
      ...
      bullets: [
        ...
        "Examinar el t\u00edtulo para determinar si hay marcas registradas (es decir, inundaci\u00f3n, salvage, junk, etc.).",
      ],
      p7Before:
    ...
    providerAttribution: {
      ...
      p1Mid: "ClearVin LLC",
      p1After:
        ", un proveedor de datos NMVTIS aprobado. Todos los derechos, t\u00edtulo e inter\u00e9s sobre los datos permanecen como propiedad de ClearVin y sus socios de datos subyacentes (DMVs estatales, NMVTIS, NHTSA, el National Insurance Crime Bureau, aseguradoras participantes, subastas salvage y contribuyentes independientes de registros de servicio).",
      p2: "CarCheckerVIN es revendedor del servicio de datos NMVTIS de ClearVin. CarCheckerVIN no modifica, edita, altera ni omite ning\u00fan valor de los datos devueltos por ClearVin; solo el dise\u00f1o y estilo del entorno se personalizan para la marca CarCheckerVIN.",
    },
    ...
    limits: {
      ...
      bullets: [
        ...
        "el estado de cumplimiento de los retiros del fabricante (solo el retiro en s\u00ed);",
...
export default async function NmvtisDisclaimerPage({ searchParams }: PageProps) {
  ...
  return (
    <div className="bg-surface min-h-[calc(100vh-200px)]">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10 sm:py-14">
        ...
        {/* Wave 11 — Spanish review banner: tells buyers the English
            version is the canonical until ClearVin's compliance team
            signs off on this translation. */}
        ...
        <div className="mt-8 space-y-6 text-sm text-slate-700 leading-relaxed">
          ...
          <p className="text-xs text-slate-500 text-center">
            {copy.alsoSee.before}
            <Link
              href={locale === "es" ? "/terms?lang=es" : "/terms"}
        ...
        <div className="mt-8 text-center">
          ...
          >
            {locale === "es" ? "Volver al pago" : "Back to checkout"}
          </Link>
        </div>
      </div>
    </div>
