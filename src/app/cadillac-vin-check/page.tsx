...
const TITLE = "Cadillac VIN Check & Decoder — Free by Model";
const DESCRIPTION =
  "Free Cadillac VIN check. Decode any Cadillac by model — Escalade, XT5, XT4, CT5, Lyriq and more — and reveal salvage, flood, buyback & accident brands. NMVTIS-backed, instant, no signup.";
...
export const metadata: Metadata = {
  ...
  keywords: [
    ...
    "decode Cadillac VIN",
    "Cadillac VIN lookup",
    "Cadillac VIN number check",
    "free Cadillac VIN check",
    "Cadillac history report",
    "Cadillac salvage check",
  ],
  alternates: { canonical: "/cadillac-vin-check" },
  openGraph: {
    title: TITLE,
    description: DESCRIPTION,
    url: `${SITE}/cadillac-vin-check`,
    type: "website",
...
export default function CadillacVinHubPage() {
  const pageUrl = `${SITE}/cadillac-vin-check`;
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Cadillac VIN Check", item: pageUrl },
    ],
  };
  ...
  const itemListSchema = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    name: "Cadillac VIN Check by Model",
    description: "Model-specific VIN check and decoder guides for popular Cadillac vehicles.",
    itemListElement: CADILLAC_MODELS.map((m, i) => ({
      "@type": "ListItem",
      position: i + 1,
      name: `${m.fullName} VIN Check`,
      url: `${SITE}/cadillac-vin-check/${m.slug}`,
    })),
  };
        ...
        <div className="bg-primary text-white">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-24 pb-14 sm:pt-28 sm:pb-20">
            <Breadcrumbs
              items={[
                { label: "Home", href: "/" },
                { label: "Cadillac VIN Check" },
              ]}
              onDark
            />

            <div className="mt-6 inline-flex items-center gap-2 bg-white/10 border border-white/20 rounded-full px-4 py-1.5 text-sm font-bold mb-4">
              <Fingerprint className="w-4 h-4" /> Cadillac VIN Decoder &amp; History
            </div>

            <h1 className="text-3xl sm:text-5xl lg:text-6xl font-headline font-extrabold leading-tight mb-4">
              Cadillac VIN Check —{" "}
              <span style={{ color: "var(--color-secondary-container)" }}>
                Decode Any Cadillac by Model
              </span>
            </h1>

            <p className="speakable-intro text-base sm:text-xl text-white/85 max-w-3xl mb-8 leading-relaxed">
              Run a free Cadillac VIN check to decode the year, engine, and
              trim of any Cadillac, and reveal salvage, flood, lemon-law buyback,
            ...
            <div className="bg-white rounded-2xl p-5 sm:p-7 shadow-xl">
              <h2 className="text-base sm:text-lg font-headline font-extrabold text-primary mb-1">
                Run a Free Cadillac VIN Check
              </h2>
              <p className="text-xs sm:text-sm text-on-surface-variant mb-4">
                Enter any 17-character Cadillac VIN
              </p>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          <section className="py-12 sm:py-16 border-b border-outline-variant">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {CADILLAC_MODELS.map((m) => (
                <Link
                  key={m.slug}
                  href={`/cadillac-vin-check/${m.slug}`}
          ...
          <section className="py-12 sm:py-16 border-b border-outline-variant">
            <h2 className="text-2xl sm:text-3xl font-headline font-extrabold text-primary mb-4">
              Why Check a Cadillac VIN?
            </h2>
            <div className="prose prose-slate max-w-none text-sm sm:text-base text-on-surface-variant leading-relaxed space-y-4">
              <p>
                Cadillac vehicles hold their value well and carry expensive,
                complex hardware — which means a branded or storm-damaged example
                can be repaired and re-sold to look like a clean luxury bargain.
                A VIN check is the fastest way to tell a genuinely clean Cadillac
                from a rebuilt one for the specific car you are considering.
              </p>
              <p>
                The first three characters of any Cadillac VIN are the World
                Manufacturer Identifier (WMI), assigned by GM by plant and
                country — Cadillac sedans commonly open with{" "}
                <strong className="text-on-surface">1G6</strong>, while SUVs and
                crossovers such as the Escalade, XT5, and Lyriq open with{" "}
                <strong className="text-on-surface">1GY</strong>. Confirming the
                WMI against the title paperwork is a simple, effective re-VIN
                check.
              </p>
              <p>
          ...
          <section className="py-10">
            <div className="rounded-3xl bg-primary p-7 sm:p-10 text-center">
              ...
              <h2 className="text-2xl sm:text-3xl font-headline font-extrabold text-white mb-2">
                Check Any Cadillac in Seconds
              </h2>
              <p className="text-white/80 text-sm sm:text-base mb-6 max-w-xl mx-auto">
                Free, instant Cadillac VIN check sourced from NMVTIS and
          ...
          <section className="py-14 sm:py-20 text-center">
            <div className="inline-flex items-center gap-2 bg-primary/10 text-primary rounded-full px-4 py-1.5 text-xs font-black uppercase tracking-wider mb-4">
              <Zap className="w-3.5 h-3.5" /> Free · Instant · Cadillac
            </div>
            <h2 className="text-2xl sm:text-4xl font-headline font-extrabold text-primary mb-3">
              One VIN. Your Cadillac&apos;s Full History. Five Seconds.
            </h2>
