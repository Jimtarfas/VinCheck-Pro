...
import CadillacVinBody from "@/components/CadillacVinBody";
import {
  CADILLAC_MODELS,
  findCadillacModel,
  cadillacFaqs,
...
export function generateStaticParams() {
  return CADILLAC_MODELS.map((m) => ({ model: m.slug }));
}
...
export async function generateMetadata({
  ...
  const m = findCadillacModel(model);
  if (!m) return {};
  ...
  return {
    ...
    alternates: { canonical: `/cadillac-vin-check/${m.slug}` },
    openGraph: {
      title,
      description,
...
export default async function CadillacModelVinPage({
  params,
}: {
  params: Promise<{ model: string }>;
}) {
  ...
  const pageUrl = `${SITE}/cadillac-vin-check/${m.slug}`;
  const faqs = cadillacFaqs(m);
  const howTo = cadillacHowTo(m);
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Cadillac VIN Check", item: `${SITE}/cadillac-vin-check` },
  ...
  return (
    <>
      ...
      <CadillacVinBody modelSlug={m.slug} />
    </>
  );
}
