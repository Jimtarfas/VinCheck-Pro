/**
 * Body for /tesla-model-x-vin-decoder — English-only Model X-specific VIN
 * decoder. Mirrors ToyotaVinLookupBody. Model X is Fremont-only (5YJ WMI),
 * launched 2015, signature falcon-wing rear doors.
 */
...
const QUICK_ANSWER_ITEMS = [
  {
    question: "How do I decode a Tesla Model X VIN?",
    answer: (
      <>
        <strong>CarCheckerVIN&apos;s</strong> free Tesla Model X VIN decoder
        queries <strong>NMVTIS</strong>, NHTSA, and Tesla VIN-format records to
        return the Gigafactory, model year, trim, and open recalls from any
        17-character Model X falcon-wing SUV VIN in seconds.
      </>
    ),
  },
  {
    question: "Where is the Tesla Model X VIN located?",
    answer: (
      <>
        The Model X VIN is printed at the lower driver-side corner of the
  ...
  {
    question: "What WMI does the Tesla Model X use?",
    answer: (
      <>
        All Model X VINs start with <code>5YJ</code> — Gigafactory California
        (Fremont). Model X has been produced exclusively at Fremont since 2015.
      </>
    ),
  },
];
...
const COPY = {
  home: "Home",
  crumb: "Tesla Model X VIN Decoder",
  badge: "Free Tesla Model X VIN Decoder · Falcon-Wing SUV Decode",
  h1Lead: "Tesla Model X VIN Decoder — ",
  h1Accent: "Decode Any Model X by Trim, Year & Recall Status.",
  intro:
  ...
  formHeading: "Free Tesla Model X VIN Decoder — Decode Any 17-Character VIN",
  formSub:
  ...
  h2Reveals: "What a Tesla Model X VIN Reveals",
  revealsIntro:
  ...
  h2Wmi: "Decoding the WMI for Tesla Model X",
  wmiIntro:
  ...
  h2YearTable: "Year Codes (10th Character) for Tesla Model X",
  yearTableIntro:
  ...
  where1Pre:
    "Every Model X prints the VIN in ",
  ...
  whereCardTitle: "Four places the Model X VIN lives",
  whereList: [
  ...
  h2Recalls: "Notable Recall Campaigns Affecting Tesla Model X",
  recallsIntro:
  ...
  recallNoteBoldLead: "Buying a used Model X?",
  recallNoteMid1: " Pair this Model X VIN decoder with a focused ",
  ...
  h2Cpo: "Tesla CPO vs a Free Model X VIN History Check",
  cpo1Pre:
    "Tesla's Certified Pre-Owned (CPO) program is dealer-direct: Model X inventory inspected at Tesla service centers, sold with a limited warranty, and listed on Tesla.com. It's a fair program — but CPO listings carry a price premium, and the inspection ",
  cpo1Bold: "does not replace a VIN-level history decode",
  cpo1Suffix:
    ". CPO confirms current condition; this page confirms what the open NMVTIS and NHTSA records say about that specific Model X.",
  cpo2Pre:
    "A free Model X VIN decode catches the things the dealer inspection isn't designed to surface: a flood brand from a state that re-titled clean, a prior salvage history (especially relevant given the cost of falcon-wing-door repair), or an open NHTSA recall the previous owner ignored. For a CPO purchase, run the decode as confirmation. For a private-party Model X, run the decode ",
  cpo2Bold: "first",
  cpo2Mid: ", then consider a full ",
  cpo2Link1: "VIN history report",
  cpo2Mid2: " and an independent inspection focused on the falcon-wing door mechanism. If anything looks off, a ",
  ...
  cpoCardTitle: "Tesla Model X pre-purchase VIN checklist",
  cpoChecklist: [
  ...
  cpoCardCta: "Run the Model X decoder first — paste the VIN here:",
  h2Internal: "Related Tesla Checks That Build On Your Model X Decode",
  internalIntro:
    "A Model X VIN decode is the entry point. These focused checks dig into specific records when something looks off — or when you want to be extra thorough before buying any used Tesla.",
  internalLinks: [
    ...
    { href: "/salvage-title-check", label: "Salvage Title Check", desc: "See if a Tesla Model X VIN carries a salvage, junk, or non-repairable title brand." },
    { href: "/flood-check", label: "Flood Damage Check", desc: "Confirm whether a Model X was branded flood or water-damaged in any state." },
  ...
  h2Faq: "Tesla Model X VIN Decoder — Frequently Asked Questions",
  faqIntro:
    "The questions buyers ask most when decoding a Tesla Model X VIN for the first time.",
  bottomBadge: "Free · Instant · NMVTIS Source",
  ctaBottomHeading: "Ready to Decode a Tesla Model X VIN?",
  ctaBottomSub:
    "Enter any 17-character Model X VIN to run a free decode against NMVTIS, the NHTSA recall feed, and Tesla's plant-WMI map. No account required.",
...
const FAQS_EN = [
  ...
  {
    question: "Where is the VIN on a Tesla Model X?",
    answer:
  ...
  {
    question: "What does a Tesla Model X VIN reveal?",
    answer:
  ...
  {
    question: "Which Gigafactory built my Tesla Model X?",
    answer:
  ...
  {
    question: "How do I check Tesla Model X history by VIN?",
    answer:
...
export default function TeslaModelXVinDecoderBody() {
  const c = COPY;
  ...
  const modelXRecalls = TESLA_NOTABLE_RECALLS.filter((r) =>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        ...
        <section className="py-10">
          <div className="rounded-3xl bg-primary p-7 sm:p-10 text-center">
            ...
            <h2 className="text-2xl sm:text-3xl font-headline font-extrabold text-white mb-2">Decode This Specific Model X VIN Right Now</h2>
            <p className="text-white/80 text-sm sm:text-base mb-6 max-w-xl mx-auto">You already have a Model X in mind. Run the VIN against NMVTIS, the NHTSA recall feed, and Tesla&apos;s plant-WMI map — free, in seconds.</p>
        ...
        <RelatedChecks exclude="/tesla-model-x-vin-decoder" />
      </div>
    </article>
  );
}
