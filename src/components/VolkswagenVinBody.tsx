/**
 * Shared body for /volkswagen-vin-check/[model].
 *
 * Each of the 10 model spokes renders from this one component, pulling
 * its facts (body style, generation, WMI prefix, VIN location, drivetrain,
 * blurb, angle, check areas, tips, FAQ, how-to) from VOLKSWAGEN_MODELS via
 * the helpers in src/lib/volkswagen-models.ts. Mirrors the section
 * structure and design tokens of LemonCheckBrandBody so the cluster looks
 * native to the site, but is tuned for model-specific VIN-check intent
 * rather than lemon-law buyback intent.
 */
...
import {
  findVolkswagenModel,
  getOtherVolkswagenModels,
  volkswagenFaqs,
  volkswagenHowTo,
} from "@/lib/volkswagen-models";
...
export default function VolkswagenVinBody({ modelSlug }: { modelSlug: string }) {
  const m = findVolkswagenModel(modelSlug);
  if (!m) return null;

  const homeHref = "/";
  const hubHref = "/volkswagen-vin-check";
  const vinCheckHref = "/vin-check";

  const others = getOtherVolkswagenModels(modelSlug, 4);
  const faqs = volkswagenFaqs(m);
  const howToSteps = volkswagenHowTo(m);

  const HEADLINE_STATS = [
    { value: m.bodyStyle, label: "Body style" },
    { value: m.generation, label: "Generation window" },
    { value: m.vinPrefix, label: "WMI prefix" },
    { value: m.segment, label: "Market segment" },
  ];

  const TRUST_STATS = [
    { icon: TRUST_ICONS[0], value: m.name, label: "model-specific decode" },
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-24 pb-14 sm:pt-28 sm:pb-20">
          <Breadcrumbs
            items={[
              ...
              { label: "Volkswagen VIN Check", href: hubHref },
              { label: m.name },
            ]}
            onDark
          />
          ...
          <p className="speakable-intro text-base sm:text-xl text-white/85 max-w-3xl mb-8 leading-relaxed">
            ...
            trim, and reveal any salvage, flood, lemon-law buyback, accident,
            or odometer-rollback brand on that exact {m.name}. Instant results
            sourced from NMVTIS and every state DMV — no signup, no credit card.
          </p>

          <div className="bg-white rounded-2xl p-5 sm:p-7 shadow-xl">
            <h2 className="text-base sm:text-lg font-headline font-extrabold text-primary mb-1">
              Run a Free {m.name} VIN Check
            </h2>
            <p className="text-xs sm:text-sm text-on-surface-variant mb-4">
              Enter the 17-character VIN from your {m.name}
            </p>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        <section className="py-12 sm:py-16 border-b border-outline-variant">
          <h2 className="text-2xl sm:text-3xl font-headline font-extrabold text-primary mb-4">
            What a {m.name} VIN Check Tells You
          </h2>
        ...
        <section className="py-12 sm:py-16 border-b border-outline-variant">
          ...
          <p className="text-sm sm:text-base text-on-surface-variant mb-7 max-w-3xl">
            A 17-character VIN identifies one specific {m.name}. Here is where it
            lives on this body style and what its characters reveal.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="rounded-2xl border border-outline-variant bg-surface-container-lowest p-5">
              ...
              <ul className="space-y-2 text-sm text-on-surface-variant">
                ...
                <li className="flex gap-2"><Check className="w-4 h-4 text-primary flex-shrink-0 mt-0.5" strokeWidth={3} /><span>WMI prefix {m.vinPrefix} identifies the Volkswagen plant and country of assembly.</span></li>
                <li className="flex gap-2"><Check className="w-4 h-4 text-primary flex-shrink-0 mt-0.5" strokeWidth={3} /><span>Model year and trim, so you can confirm the listing matches the real {m.name}.</span></li>
              </ul>
            </div>
          </div>
        </section>
        ...
        <section className="py-12 sm:py-16 border-b border-outline-variant">
          <h2 className="text-2xl sm:text-3xl font-headline font-extrabold text-primary mb-2">
            Smart Buyer Tips for the {m.name}
          </h2>
          <p className="text-sm sm:text-base text-on-surface-variant mb-7 max-w-3xl">
            Model-specific pointers that make a problem {m.name} easier to catch
            before you sign.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {m.tips.map((tip, i) => (
        ...
        <section className="py-12 sm:py-16 border-b border-outline-variant">
          <h2 className="text-2xl sm:text-3xl font-headline font-extrabold text-primary mb-2">
            How to Check a {m.name} VIN — 6 Steps
          </h2>
        ...
        <section className="py-12 sm:py-16 border-b border-outline-variant">
          <h2 className="text-2xl sm:text-3xl font-headline font-extrabold text-primary mb-4">
            Is the Volkswagen {m.name} Reliable?
          </h2>
          <div className="prose prose-slate max-w-none text-sm sm:text-base text-on-surface-variant leading-relaxed space-y-4">
            <p>
              Reliability is a{" "}
              <strong className="text-on-surface">per-vehicle question</strong>,
              not a per-model verdict. Volkswagen builds large volumes of
              trouble-free {m.name}s, and even a model year with many NHTSA
            ...
            <p>
              ...
              High complaint clusters describe a model year, not the specific car
              in front of you — which is exactly why a{" "}
              <Link href={vinCheckHref} className="text-primary font-bold hover:underline">
                VIN-level history check
              </Link>{" "}
              beats model reputation: it tells you about the one {m.name} you are
              about to buy.
            </p>
          </div>
        </section>
        ...
        <section className="py-10">
          <div className="rounded-3xl bg-primary p-7 sm:p-10 text-center">
            ...
            <p className="text-white/80 text-sm sm:text-base mb-6 max-w-xl mx-auto">
              Free, instant {m.name} VIN check sourced from NMVTIS and every
        ...
        <section className="py-12 sm:py-16 border-b border-outline-variant">
          <h2 className="text-2xl sm:text-3xl font-headline font-extrabold text-primary mb-2">
            VIN Checks for Other Volkswagen Models
          </h2>
          <p className="text-sm sm:text-base text-on-surface-variant mb-7">
            VIN locations and decode details differ by body style. Compare the
            {" "}{m.name} with these model guides, or browse every Volkswagen model.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {others.map((o) => (
              ...
              >
                ...
                <div>
                  <div className="text-sm font-bold text-primary group-hover:underline">{o.name} VIN Check</div>
            ...
            <Link href={hubHref} className="flex items-start gap-3 rounded-xl border border-outline-variant bg-surface hover:border-primary/40 hover:bg-primary/5 transition-colors p-4 group">
              ...
              <div>
                <div className="text-sm font-bold text-primary group-hover:underline">All Volkswagen Models</div>
                <div className="text-xs text-on-surface-variant mt-0.5">Browse every Volkswagen VIN guide.</div>
              </div>
            </Link>
        ...
        <section className="py-14 sm:py-20 text-center">
          ...
          <p className="text-sm sm:text-base text-on-surface-variant max-w-2xl mx-auto mb-8">
            ...
            when the paper title looks clean. Run the free {m.name} check before
            you write a check.
          </p>
