/**
 * Shared body for /chevrolet-vin-check/[model].
 *
 ...
 * blurb, angle, check areas, tips, FAQ, how-to) from CHEVY_MODELS via the
 * helpers in src/lib/chevy-models.ts. Mirrors the section structure and
 * design tokens of LemonCheckBrandBody so the cluster looks native to the
 * site, but is tuned for model-specific VIN-check intent rather than
 * lemon-law buyback intent.
 */
...
export default function ChevyVinBody({ modelSlug }: { modelSlug: string }) {
  const m = findChevyModel(modelSlug);
  if (!m) return null;

  const homeHref = "/";
  const hubHref = "/chevrolet-vin-check";
  const vinCheckHref = "/vin-check";
  const wmiPrefix = m.vinPrefix.split(" / ")[0];

  const others = getOtherChevyModels(modelSlug, 4);
  const faqs = chevyFaqs(m);
  const howToSteps = chevyHowTo(m);
  ...
  return (
    <article className="pb-16 bg-surface">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-24 pb-14 sm:pt-28 sm:pb-20">
          <Breadcrumbs
            items={[
              ...
              { label: "Chevrolet VIN Check", href: hubHref },
              { label: m.name },
            ]}
            onDark
          />
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="rounded-2xl border border-outline-variant bg-surface-container-lowest p-5">
              ...
              <ul className="space-y-2 text-sm text-on-surface-variant">
                ...
                <li className="flex gap-2"><Check className="w-4 h-4 text-primary flex-shrink-0 mt-0.5" strokeWidth={3} /><span>WMI prefix {m.vinPrefix} identifies the GM plant and country of assembly.</span></li>
        ...
        <section className="py-12 sm:py-16 border-b border-outline-variant">
          <h2 className="text-2xl sm:text-3xl font-headline font-extrabold text-primary mb-4">
            Is the Chevrolet {m.name} Reliable?
          </h2>
          <div className="prose prose-slate max-w-none text-sm sm:text-base text-on-surface-variant leading-relaxed space-y-4">
            <p>
              Reliability is a{" "}
              <strong className="text-on-surface">per-vehicle question</strong>,
              not a per-model verdict. Chevrolet builds large volumes of
        ...
        <section className="py-12 sm:py-16 border-b border-outline-variant">
          <h2 className="text-2xl sm:text-3xl font-headline font-extrabold text-primary mb-2">
            VIN Checks for Other Chevrolet Models
          </h2>
          <p className="text-sm sm:text-base text-on-surface-variant mb-7">
            ...
            {" "}{m.name} with these model guides, or browse every Chevrolet model.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <Link href={hubHref} className="flex items-start gap-3 rounded-xl border border-outline-variant bg-surface hover:border-primary/40 hover:bg-primary/5 transition-colors p-4 group">
              ...
              <div>
                <div className="text-sm font-bold text-primary group-hover:underline">All Chevrolet Models</div>
                <div className="text-xs text-on-surface-variant mt-0.5">Browse every Chevrolet VIN guide.</div>
              </div>
            </Link>
