...
const ROWS_EN: { feature: string; us: boolean | string; them: boolean | string }[] = [
  { feature: "Single report price", us: "$7.99", them: "Subscription only" },
...
const ROWS_ES: typeof ROWS_EN = [
  { feature: "Precio de reporte único", us: "$7.99", them: "Solo suscripción" },
...
const COPY = {
  en: {
    ...
    price2: "CarCheckerVIN is intentionally pay-as-you-go: $7.99 for a single report, $14.99 for a three-pack, and $24.99 for a month of unlimited access — with no auto-renewal. If you only need one report this year, you pay $7.99 and you are done. If you need unlimited for a month while shopping, you pay $24.99 once and it expires on its own.",
    ...
    m2: "CarCheckerVIN does not offer a continuous monitoring service today. Our model is a fresh, point-in-time report you can re-run any time you need an update — for $7.99 each time. For most owners, an annual re-check is more than sufficient and is cheaper than 12 months of subscription.",
  ...
  es: {
    ...
    price2: "CarCheckerVIN es intencionalmente pago por uso: $7.99 por un reporte único, $14.99 por un paquete de tres, y $24.99 por un mes de acceso ilimitado — sin auto-renovación. Si solo necesitas un reporte este año, pagas $7.99 y listo. Si necesitas ilimitado por un mes mientras compras, pagas $24.99 una vez y expira por sí solo.",
    ...
    m2: "CarCheckerVIN no ofrece un servicio de monitoreo continuo hoy. Nuestro modelo es un reporte fresco, en un punto en el tiempo, que puedes volver a ejecutar cada vez que necesites una actualización — por $7.99 cada vez. Para la mayoría de propietarios, una re-verificación anual es más que suficiente y es más barata que 12 meses de suscripción.",
...
const FAQS_EN = [
  ...
  { question: "Can I get a one-time report instead of a subscription?", answer: "With CarCheckerVIN, yes. It is intentionally pay-as-you-go: $7.99 for a single report, $14.99 for a three-pack, or $24.99 for a 30-day unlimited pass that simply expires on its own with no auto-renewal. Bumper does not offer an equivalent one-time report — its consumer model is built around an ongoing monthly subscription that you must remember to cancel." },
...
const FAQS_ES = [
  ...
  { question: "¿CarCheckerVIN es más barato que Bumper?", answer: "Para un solo vehículo, sí. CarCheckerVIN cobra $7.99 por un reporte sin suscripción, mientras Bumper requiere una suscripción mensual de aproximadamente $19.99. Si solo necesitas un reporte este año, pagas $7.99 una vez y listo. Una suscripción olvidada de Bumper puede costar silenciosamente $200 a $240 al año, así que para compradores únicos CarCheckerVIN es el camino más barato." },
  ...
  { question: "¿Puedo obtener un reporte único en lugar de una suscripción?", answer: "Con CarCheckerVIN, sí. Es intencionalmente pago por uso: $7.99 por un reporte único, $14.99 por un paquete de tres, o $24.99 por un pase ilimitado de 30 días que simplemente expira por sí solo sin auto-renovación. Bumper no ofrece un reporte único equivalente — su modelo de consumidor está construido alrededor de una suscripción mensual continua que debes recordar cancelar." },
  { question: "¿Cuál tiene mejor valor, CarCheckerVIN o Bumper?", answer: "Depende de tu necesidad. Para el comprador típico de auto usado cuya búsqueda dura de dos a cuatro semanas, CarCheckerVIN ofrece mejor valor: datos vehiculares comparables por un reporte único de $7.99 sin cargo recurrente. Bumper ofrece mejor valor si posees múltiples vehículos y quieres monitoreo continuo con alertas, o usarás sus complementos de búsqueda de personas regularmente lo suficiente para justificar la tarifa mensual." },
];
