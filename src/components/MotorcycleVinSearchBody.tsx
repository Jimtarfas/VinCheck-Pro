...
const COPY = {
  ...
  fr: {
    home: "Accueil", tools: "Outils", crumb: "Recherche VIN de moto",
    heroBadge: "100% gratuit · Décodage instantané · Sans inscription",
    h1: "Recherche VIN de moto",
    heroPre: "Décodeur VIN de moto gratuit. Saisis n'importe quel VIN de moto de 17 caractères pour identifier instantanément le ",
    heroBold: "fabricant, pays d'origine, année modèle, code d'usine et séquence de production",
    heroPost: " — plus une décomposition position par position de ce que signifie chaque caractère. Fonctionne pour Harley-Davidson, Honda, Yamaha, Suzuki, Kawasaki, BMW, Ducati, Triumph, KTM et toutes les grandes marques de motos.",
    tocAria: "Sur cette page",
    toc: [
      { id: "tool", label: "Outil de recherche" },
      { id: "features", label: "Fonctionnalités" },
      { id: "find-vin", label: "Trouve ton VIN" },
      { id: "how-vin-works", label: "Comment fonctionne un VIN" },
      { id: "brands", label: "Marques prises en charge" },
      { id: "faq", label: "FAQ" },
    ],
    trustStats: [
      { icon: Bike, value: "25+", label: "marques de motos" },
      { icon: Zap, value: "Instantané", label: "décodage côté client" },
      { icon: Globe2, value: "ISO 3779", label: "norme 17 caractères" },
      { icon: BadgeCheck, value: "Gratuit", label: "sans inscription" },
    ],
    toolH2: "Décode un VIN de moto",
    toolSub: "Saisis n'importe quel VIN de moto de 17 caractères — nous décoderons le fabricant, l'année, l'usine et la séquence de production instantanément",
    toolFoot: "Gratuit · Sans inscription · Décodage instantané",
    featuresH2: "Fonctionnalités du décodeur VIN de moto gratuit",
    featuresIntro: "Construit spécifiquement pour les motos — reconnaît les fabricants et les particularités que les outils génériques de VIN de voiture manquent.",
    features: [
      { icon: Zap, title: "Décodage hors ligne instantané", desc: "Le WMI, l'année, l'usine et la série sont décodés côté client — les résultats apparaissent en millisecondes sans aucun appel API." },
      { icon: Bike, title: "Plus de 25 marques de motos", desc: "Reconnaît Harley-Davidson, Honda, Yamaha, Suzuki, Kawasaki, BMW, Ducati, Triumph, KTM, Royal Enfield et plus encore." },
      { icon: Globe2, title: "Pays et région d'origine", desc: "Décode le code de région du WMI pour que tu connaisses le pays et le continent où ta moto a été assemblée." },
      { icon: Calendar, title: "Désambiguïsation de l'année modèle", desc: "Gère le cycle de lettres de 30 ans et montre les deux années possibles quand le code est ambigu." },
      { icon: Hash, title: "Décomposition visuelle du VIN", desc: "Un affichage position par position codé par couleur t'apprend à lire n'importe quel VIN de moto à vue." },
      { icon: ShieldCheck, title: "Historique complet en un clic", desc: "Décode gratuitement, puis saute directement au rapport d'historique complet (titre, vol, accidents, rappels)." },
    ],
    findVinH2: "Où se trouve le VIN sur une moto ?",
    findVinIntro: "Les fabricants estampent le VIN à plusieurs endroits sur chaque moto, en plus de l'imprimer sur chaque document papier lié au véhicule. Savoir où chercher fait gagner du temps et confirme que le VIN n'a pas été altéré — un VIN manquant ou rayé est l'un des indicateurs les plus forts d'une moto volée.",
    findVinLocations: [
      { title: "Colonne de direction (tête de cadre)", desc: "L'emplacement le plus courant sur les sportives japonaises (Honda, Yamaha, Kawasaki, Suzuki) et la plupart des marques européennes. Cherche du côté droit juste sous le guidon." },
      { title: "Côté droit de la colonne de cadre (Harley-Davidson)", desc: "Harley estampe le VIN profondément du côté droit de la colonne de cadre. Les Harley plus anciennes l'estampaient aussi du côté gauche du carter moteur." },
      { title: "Carter moteur", desc: "La plupart des motos modernes estampent aussi le VIN (ou un VIN partiel) sur le bloc moteur, typiquement près de la culasse ou du démarreur." },
      { title: "Cadre près du bras oscillant", desc: "Beaucoup de cruisers et de motos d'aventure portent un marquage VIN secondaire sur le cadre au-dessus ou à côté du pivot du bras oscillant." },
      { title: "Titre, immatriculation et documents d'assurance", desc: "Chaque document papier lié à la moto liste le VIN. Utilise-le pour recouper avec le VIN estampé sur le cadre." },
    ],
    howVinH2: "Comment fonctionne un VIN de moto (17 caractères expliqués)",
    howVinIntro: "Le format VIN de 17 caractères est une norme internationale (ISO 3779) adoptée en 1981. Chaque moto construite depuis pour la vente en Amérique du Nord, en Europe et sur la plupart des autres marchés utilise ce format. Les lettres I, O et Q sont exclues pour éviter la confusion avec les chiffres 1 et 0.",
    sampleVinLabel: "Exemple — un VIN Harley-Davidson, décodé",
    sampleVinFoot: "Chaque bloc correspond à un groupe de positions ci-dessous. Lis de gauche à droite : qui l'a construite, ce que c'est, un checksum, l'année, l'usine et la série unique.",
    vinPositions: [
      { pos: "1–3", title: "WMI — Identifiant Mondial du Fabricant", desc: "Identifie le pays, le fabricant et la catégorie de moto. La position 1 est la région (1–5 = Amérique du Nord, J–R = Asie, S–Z = Europe). Exemples : 1HD = Harley-Davidson, JH2 = Honda, JYA = Yamaha, ZDM = Ducati." },
      { pos: "4–8", title: "VDS — Section Descriptive du Véhicule", desc: "Encode les attributs spécifiques au modèle : cylindrée et configuration du moteur, type de carrosserie ou de cadre, transmission et finition. Chaque fabricant utilise son propre schéma dans ces cinq positions." },
      { pos: "9", title: "Chiffre de contrôle", desc: "Un checksum mathématique qui valide le VIN entier. Si même un caractère est faux, le chiffre de contrôle ne correspondra pas — utile pour attraper les erreurs de transcription." },
      { pos: "10", title: "Année modèle", desc: "Un seul caractère qui encode l'année modèle. Les lettres A–Y (sautant I, O, Q, U, Z) cyclent sur 30 ans ; les chiffres 1–9 couvrent 2001–2009. Le même code peut signifier deux années possibles à 30 ans d'intervalle." },
      { pos: "11", title: "Usine d'assemblage", desc: "Un seul caractère identifiant l'usine spécifique où la moto a été construite. Chaque fabricant attribue ses propres codes d'usine." },
      { pos: "12–17", title: "Séquence de production", desc: "Un numéro de série unique de six caractères attribué par le fabricant. Combiné avec le WMI et l'année, il identifie de façon unique ta moto individuelle parmi des millions." },
    ],
    posWord: "Pos",
    pairWithPre: "Combine un décodage VIN avec une ",
    pairWithLink1: "vérification VIN de moto",
    pairWithMid: " complète pour voir le statut du titre, les dossiers de vol, accidents et rappels. Pour la fiche originale de spécifications d'usine, utilise le ",
    pairWithLink2: "Générateur de Window Sticker",
    pairWithPost: ".",
    midCtaH2: "Décode n'importe quel VIN de moto maintenant",
    midCtaSub: "Gratuit, instantané, sans inscription. Identifie le fabricant, l'année, le pays et l'usine en millisecondes — puis saute au rapport d'historique complet quand tu es prêt.",
    midCtaBtn: "Chercher un VIN",
    brandsH2: "Marques de motos prises en charge",
    brandsIntro: "Le décodeur reconnaît plus de 25 WMI de fabricants de motos et se rabat sur un décodage générique pour les marques non reconnues.",
    supportedBrands: [
      "Harley-Davidson", "Honda", "Yamaha", "Suzuki", "Kawasaki", "BMW Motorrad",
      "Ducati", "Triumph", "KTM", "Indian", "Royal Enfield", "Aprilia",
      "MV Agusta", "Husqvarna", "Moto Guzzi", "Piaggio / Vespa", "Buell", "Victory (Polaris)",
    ],
    internalH2: "Plus d'outils VIN qui se combinent avec une recherche VIN de moto",
    internalIntro: "Décoder le VIN est la première étape. Ces vérifications complètent l'histoire de la moto.",
    internalLinks: [
      { href: "/motorcycle-vin-check", label: "Vérification VIN de moto complète", desc: "Statut du titre, dossiers de vol, accidents, rappels et privilèges pour n'importe quelle moto." },
      { href: "/vin-decoder", label: "Décodeur VIN", desc: "Décode n'importe quel VIN de 17 caractères — voitures et motos — en marque, année, usine et séquence." },
      { href: "/stolen-vehicle-check", label: "Vérification de véhicule volé", desc: "Vérifie le VIN contre la base de données de vol du NICB avant d'acheter." },
      { href: "/window-sticker", label: "Générateur de Window Sticker", desc: "Recrée la fiche originale de spécifications d'usine pour la moto." },
      { href: "/vin-check", label: "Vérification complète d'historique VIN", desc: "Dossiers de titre, accidents, odomètre et rappels dans un seul rapport." },
      { href: "/build-sheet", label: "Fiche de construction par VIN", desc: "Obtiens le dossier original de fabrication et les codes d'options." },
    ],
    faqH2: "Recherche VIN de moto — Foire aux questions",
    faqIntro: "Les questions que les motards posent le plus sur le décodage d'un VIN de moto.",
    bottomBadge: "Gratuit · Instantané · Sans inscription",
    bottomH2: "Décode n'importe quel VIN de moto maintenant",
    bottomSub: "Gratuit, instantané, sans inscription. Décode le fabricant, l'année, le pays, l'usine et la séquence de production — puis saute au rapport d'historique de moto complet quand tu es prêt.",
    bottomBtn: "Chercher un VIN",
  },
} as const;
