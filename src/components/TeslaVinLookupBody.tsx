/**
 * Body for /tesla-vin-lookup — English-only TOP-LEVEL HUB page targeting
 * the broad "tesla vin lookup" keyword. Emphasizes the lookup intent
...
const QUICK_ANSWER_ITEMS = [
  {
    question: "What is a Tesla VIN lookup?",
    answer: (
      <>
        <strong>CarCheckerVIN&apos;s</strong> free Tesla VIN lookup queries{" "}
  ...
  {
    question: "Is the Tesla VIN lookup free?",
    answer: (
      <>
        Yes. CarCheckerVIN&apos;s Tesla VIN lookup is free with no sign-up or
...
const COPY = {
  home: "Home",
  crumb: "Tesla VIN Lookup",
  badge: "Free Tesla VIN Lookup   ·   NMVTIS + NHTSA   ·   No Sign-Up",
  h1Lead: "Tesla VIN Lookup — ",
  h1Accent: "Title Brands, Recalls & Full History Free.",
  intro:
    "CarCheckerVIN's free Tesla VIN lookup queries NMVTIS, the NHTSA Vehicle Safety Database, all 50 state DMVs, and salvage auctions for any 17-character Tesla VIN. As an NMVTIS-approved data provider, CarCheckerVIN returns title brands (Salvage, Flood, Rebuilt), open NHTSA recalls, accident history, odometer integrity, ownership transfers, and Gigafactory-of-origin for every Model S, Model 3, Model X, Model Y, Cybertruck, and Roadster — free, no credit card. Drop any 17-character Tesla VIN into the form below — 5YJ from Fremont, 7SAY from Austin, LRW from Shanghai, or XP7 from Berlin — and the Tesla VIN lookup returns the full picture in seconds. No sign-up, no card, no catch.",
  formHeading: "Free Tesla VIN Lookup — Search Any 17-Character Tesla VIN",
  formSub:
  ...
  h2How: "How a Tesla VIN Lookup Works",
  howIntro:
    "A Tesla VIN lookup is a three-step job. You supply the 17-character VIN; the lookup reads the WMI for the Gigafactory, position 10 for the year, the VDS for the model and trim, then queries NMVTIS for title brands, the NHTSA recall feed for open Tesla campaigns, and the state-by-state title chain for ownership transfers. The full lookup runs in seconds against the same VIN-keyed records Tesla service centers, dealers, and insurers use.",
  howSteps: [
    {
      tag: "Step 1",
      title: "Enter the Tesla VIN",
      body:
        "Paste the 17-character VIN from the windshield, the door pillar sticker, the Tesla app, or the title document. The Tesla VIN lookup validates that it is exactly 17 characters and excludes the letters I, O, and Q per FMVSS 565 before running.",
    },
    {
      tag: "Step 2",
    ...
    {
      tag: "Step 3",
      title: "Read the Tesla VIN lookup report",
      body:
  ...
  h2Reveal: "What a Tesla VIN Lookup Reveals",
  revealIntro:
    "A Tesla VIN lookup is more than a decoder. It is the same VIN-keyed data Tesla service centers, dealers, insurers, and state DMVs already use, presented for a buyer rather than a back-office system. Here is what comes back when you lookup a Tesla VIN with CarCheckerVIN.",
  ...
  reveal1Suffix: ". From those alone the Tesla VIN lookup returns the year, model, trim, drivetrain, and factory equipment.",
  ...
  revealCardNote: "One 17-character Tesla VIN, three layers of insight. The full Tesla VIN lookup runs in seconds and never asks for an account.",
  h2Models: "Every Tesla Model the Lookup Covers",
  modelsIntro:
    "The Tesla VIN lookup handles every production Tesla shipped since 2008 — six models across four Gigafactories. The lookup reads the VDS (positions 4-8) to identify the model and trim and then surfaces model-specific recalls, common defect patterns, and title-brand prevalence.",
  h2Plants: "Every Gigafactory the Lookup Maps",
  plantsIntro:
    "The first three characters of any Tesla VIN — the World Manufacturer Identifier (WMI) — tell the lookup where the car was built. Tesla uses four WMIs, one per Gigafactory, under SAE J853. Every Tesla VIN lookup starts here.",
  h2Where: "Where to Find Your Tesla VIN Before You Lookup",
  where1Pre: "Most people get stuck before they even start a Tesla VIN lookup because they cannot find the VIN. Good news — every Tesla prints the VIN in ",
  ...
  where3:
    "If the VIN on the dashboard does not match the VIN on the Tesla title, stop. That mismatch is a strong signal that something is wrong with the vehicle's identity — exactly the kind of thing a Tesla VIN lookup is designed to catch via the federal check-digit at position 9.",
  ...
  whereCardNote: "Found it? Drop the VIN into the form above and run a free Tesla VIN lookup against NMVTIS in seconds.",
  ...
  quirks: [
    { title: "Battery health is invisible from the VIN", body: "A Tesla VIN lookup confirms the trim and approximate build date, but battery state-of-health (SoH) requires a Tesla service-center diagnostic. Used Tesla buyers should request a Tesla service report alongside the VIN lookup — the two together give you the title side and the battery side of the picture." },
    { title: "Autopilot hardware HW3 vs HW4 matters for FSD", body: "Tesla shipped HW3 from 2019-early 2023 and transitioned to HW4 starting in April 2023. The Tesla VIN lookup confirms the model year, which narrows hardware tier — and HW3 vs HW4 determines whether Full Self-Driving can be transferred between cars under Tesla's transfer rules." },
    { title: "FSD transfer rules change", body: "Tesla's policy on transferring previously-purchased Full Self-Driving (FSD) between vehicles has changed multiple times. A Tesla VIN lookup confirms the model and approximate build era, which sets the baseline for what FSD transfer is available — but always check Tesla's current policy at purchase." },
    { title: "Supercharging history follows the VIN", body: "Tesla's Supercharger session log is VIN-keyed and survives ownership transfers. A used Tesla you buy comes with every Supercharge session the prior owner logged, accessible through the Tesla app once the vehicle is associated with your account. The Tesla VIN lookup confirms the VIN before you take delivery." },
  ],
  h2Recalls: "Notable Open Tesla Recalls the Lookup Surfaces",
  recallIntro:
    `The Tesla VIN lookup queries the live NHTSA recall feed for the VIN you entered. ${TESLA_RECALL_OVERVIEW.totalCampaignsThrough2026} ${TESLA_RECALL_OVERVIEW.otaShare} ${TESLA_RECALL_OVERVIEW.freeRepair}`,
  h2Signs: "When You Should Run a Tesla VIN Lookup",
  signsIntro:
    "A Tesla VIN lookup is cheap insurance — actually free — for anyone making a decision about a specific Tesla. Six situations where the Tesla VIN lookup pays off before you commit.",
  signs: [
    { title: "Before buying a used Tesla private-party", body: "The seller's word is not the title record. A free Tesla VIN lookup confirms the Gigafactory, year, trim, and surfaces title brands or open recalls the seller may not even know about." },
    ...
    { title: "Decoding an unfamiliar Tesla trim", body: "Need to know if that used Model 3 is Standard Range, Long Range, Performance, or the Highland refresh? The decoder portion of the Tesla VIN lookup returns the factory-installed trim." },
    { title: "Checking an inherited Tesla", body: "Inheriting or being gifted a Tesla? A free Tesla VIN lookup confirms the title status and surfaces any open recalls before you put your name on the registration." },
    { title: "Verifying an insurance quote on a Tesla", body: "Insurers price by VIN. Running the Tesla VIN lookup yourself confirms the year, trim, and safety equipment they used — and catches mistakes that inflate premiums." },
    { title: "Spotting a too-good-to-be-true Tesla deal", body: "A clean-looking Model S, 3, X, Y, or Cybertruck priced well below market is the classic salvage-or-flood tell. Teslas don't depreciate fast — if the price is off, the Tesla VIN lookup is the fastest way to confirm or rule it out." },
  ],
  h2Internal: "Related Tesla VIN Pages That Build On the Lookup",
  internalIntro:
    "The Tesla VIN lookup is the broad entry point. These focused Tesla pages dig deeper into a specific Gigafactory, a specific model, or a specific record type — recalls, title brands, history.",
  internalLinks: [
  ...
  h2Faq: "Tesla VIN Lookup — Frequently Asked Questions",
  ...
  return (
    <article className="pb-16 bg-surface">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        ...
        <RelatedChecks exclude="/tesla-vin-lookup" />
      </div>
    </article>
  );
}
