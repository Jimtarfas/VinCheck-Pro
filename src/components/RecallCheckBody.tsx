...
const COPY = {
  ...
  fr: {
    home: "Accueil", crumb: "Vérification de rappels",
    badge: "Rappels de sécurité NHTSA",
    h1Lead: "Vérification VIN des rappels — ",
    h1Accent: "Rappels de sécurité NHTSA ouverts",
    intro: "Découvre si un véhicule a des rappels de sécurité non réparés avant de le conduire ou de l'acheter. Saisis un VIN de 17 caractères pour voir le composant affecté, le numéro de campagne, le défaut, le risque de sécurité et la réparation gratuite chez le concessionnaire. Aperçu gratuit, sans carte de crédit, résultats en moins de 5 secondes.",
    formHeading: "Vérifie les rappels de sécurité ouverts par VIN",
    formSub: "Saisis n'importe quel VIN de 17 caractères — voitures, camions, motos, VR",
    secureNote: "Chiffré 256 bits · Conforme DPPA · Aucune donnée personnelle stockée",
    trustStats: [
      { icon: Siren, value: "NHTSA", label: "données officielles de rappels" },
      { icon: ShieldAlert, value: "Ouverts", label: "campagnes non réparées signalées" },
      { icon: Clock, value: "< 5 sec", label: "temps moyen de rapport" },
      { icon: BadgeCheck, value: "Aperçu gratuit", label: "sans carte de crédit" },
    ],
    statsHeading: "Vérification VIN des rappels — En chiffres",
    headlineStats: [
      { value: "NHTSA", label: "Base de données officielle des rappels américains" },
      { value: "Par VIN", label: "Correspondance à la plage exacte de fabrication" },
      { value: "6 champs", label: "Capturés par campagne de rappel" },
      { value: "< 5 sec", label: "Temps moyen de décodage VIN" },
      { value: "0 $", label: "Coût de l'aperçu gratuit" },
    ],
    h2Why: "Pourquoi un rappel ouvert est la réparation gratuite que la plupart des propriétaires ne réclament jamais",
    whyPre: "Un rappel de sécurité est émis lorsqu'un fabricant ou la ",
    whyNhtsa: "National Highway Traffic Safety Administration (NHTSA)",
    whyMid1: " constate qu'un véhicule a un défaut qui crée un risque de sécurité déraisonnable ou échoue à une norme fédérale de sécurité. Le constructeur est alors légalement tenu de notifier les propriétaires et de réparer le défaut ",
    whyBoldFree: "sans frais",
    whyMid1End: ". Le hic est que l'avis ne parvient qu'à ceux que le fabricant peut trouver, donc les voitures d'occasion qui ont changé de mains portent souvent des rappels ouverts dont le conducteur actuel n'a jamais entendu parler.",
    why2: "Cet écart est dangereux et courant. Les rappels couvrent les gonfleurs d'airbag qui peuvent éclater, les systèmes de carburant qui peuvent prendre feu, les freins qui peuvent défaillir et les logiciels qui peuvent désactiver les fonctionnalités de sécurité. Quand un véhicule est vendu de manière privée, le rappel ouvert se transfère avec lui silencieusement. Rien sur le titre ou l'annonce ne te prévient, et le vendeur peut aussi ne pas le savoir.",
    why3: "Une vérification VIN des rappels comble cet écart. Elle confronte le VIN exact de 17 caractères à la base de données NHTSA et retourne chaque campagne ouverte au dossier, donc tu sais ce qui ne va pas, à quel point c'est sérieux et que la réparation t'attend gratuitement chez n'importe quel concessionnaire franchisé.",
    h2Record: "Ce que montre chaque registre de rappel",
    recordIntro: "Pour chaque campagne ouverte au dossier, le rapport capture les détails dont tu as besoin pour comprendre le risque et réserver la réparation.",
    recordFields: [
      { title: "Composant affecté", desc: "Le système exact sous rappel : gonfleurs d'airbag, freins, système de carburant, direction, câblage ou logiciel de sécurité." },
      { title: "Numéro de campagne NHTSA", desc: "L'ID officiel de la campagne qu'un concessionnaire utilise pour commander les bonnes pièces et consigner la réparation. Apporte-le avec le VIN." },
      { title: "Résumé du défaut", desc: "Une description en langage simple de ce qui ne va pas et des conditions dans lesquelles le défaut peut se produire." },
      { title: "Risque de sécurité", desc: "La conséquence que le rappel vise à prévenir, comme l'incendie, la perte de direction ou la rupture d'airbag et blessure." },
      { title: "Réparation gratuite", desc: "La réparation, le remplacement ou le remboursement que le fabricant doit fournir sans frais au propriétaire chez n'importe quel concessionnaire franchisé." },
      { title: "Fabricant et date", desc: "Quel constructeur a émis la campagne et quand, pour que tu puisses évaluer à quel point le rappel est récent et si les pièces sont disponibles." },
    ],
    h2OpenVs: "Ouvert vs. complété, et rappel vs. bulletin de service",
    openVsIntro: "Deux distinctions décident de ce qu'un registre de rappel signifie réellement pour toi : si la réparation a été faite, et si tu regardes même un rappel.",
    openCardTitle: "Rappel ouvert",
    openCardBody: "Un défaut de sécurité est confirmé pour ce VIN et la réparation gratuite n'a pas encore été effectuée.",
    openCardBullets: ["La réparation est disponible et gratuite, juste pas faite", "Se transfère avec la voiture à chaque nouveau propriétaire", "Un point de négociation, et parfois un avertissement de ne pas conduire"],
    doneCardTitle: "Rappel complété et TSB",
    doneCardBody: "Un rappel complété a déjà été corrigé et consigné. Un bulletin de service (TSB) documente une réparation connue mais ne porte aucune obligation de réparation gratuite.",
    doneCardBullets: ["Garde le reçu de service du concessionnaire comme preuve d'achèvement", "Permets un court délai avant qu'une réparation n'apparaisse comme complétée", "Un TSB est une orientation, pas un rappel, et n'est généralement pas gratuit"],
    h2Steps: "Comment vérifier un VIN pour les rappels — Étape par étape",
    stepsIntro: "Vérifier et agir sur les rappels prend moins de deux minutes.",
    steps: [
      { step: "01", title: "Localise le VIN", body: "Le VIN de 17 caractères est sur le tableau de bord à travers le pare-brise inférieur, l'autocollant du montant de porte conducteur et le titre. Confirme que les trois correspondent avant de te fier à un résultat." },
      { step: "02", title: "Exécute le VIN ci-dessus", body: "Saisis le VIN. La recherche le confronte aux données de campagne NHTSA, y compris les plages spécifiques de date de fabrication et d'usine que chaque rappel cible, donc tu vois uniquement les rappels qui s'appliquent à cette voiture exacte." },
      { step: "03", title: "Lis chaque campagne ouverte", body: "Pour chaque rappel ouvert, examine le composant affecté, le numéro de campagne, le résumé du défaut et le risque de sécurité. Le numéro de campagne est ce dont un concessionnaire a besoin pour commander les pièces." },
      { step: "04", title: "Sépare ouvert de complété", body: "Confirme quels rappels sont encore ouverts et lesquels ont été corrigés. Si le vendeur prétend qu'un rappel a été réparé, demande le reçu de service daté plutôt que de te fier à la base de données seule." },
      { step: "05", title: "Réserve la réparation gratuite", body: "Apporte le VIN et le numéro de campagne à n'importe quel concessionnaire franchisé de la marque. La réparation est effectuée sans frais, même si tu n'es pas le propriétaire original. Si les pièces ne sont pas prêtes, demande à rejoindre la liste d'attente." },
    ],
    h2Priority: "Rappels que tu ne devrais jamais ignorer",
    priorityIntro: "Tout rappel ouvert vaut la peine d'être réparé, mais ces catégories portent le risque de blessure le plus élevé et méritent une action immédiate.",
    priorities: [
      { flag: "Gonfleur d'airbag Takata", desc: "Gonfleurs qui peuvent éclater et projeter des fragments de métal sur les occupants. Le plus grand rappel de l'histoire américaine ; traite-le comme urgent, à réparer avant de conduire." },
      { flag: "Système de carburant ou risque d'incendie", desc: "Fuites, pompes à carburant défectueuses ou câblage pouvant prendre feu. Les avertissements de garer dehors jusqu'à la réparation accompagnent souvent ces campagnes." },
      { flag: "Défaillance des freins ou de la direction", desc: "La perte de contrôle des freins ou de la direction est un risque direct d'accident. Ne retarde pas la réparation sur ceux-ci." },
      { flag: "Pannes électriques et logicielles", desc: "Calage, accélération non intentionnelle ou systèmes de sécurité désactivés par des défauts électroniques ou de firmware." },
      { flag: "Défaut de ceinture ou système de retenue", desc: "Ceintures pouvant ne pas se verrouiller ou retenir correctement lors d'un accident, réduisant la protection des occupants." },
      { flag: "Réparation pas encore disponible", desc: "Un rappel ouvert avec des pièces encore en production. Rejoins la liste d'attente du concessionnaire et demande les mesures de sécurité intérimaires." },
    ],
    priorityBoldLead: "En résumé :",
    priorityBoldRest: " une réparation de rappel est gratuite, légalement garantie et liée à ton VIN peu importe combien de propriétaires la voiture a eu. Le seul coût d'un rappel ouvert est le risque de le laisser non réparé.",
    midCtaHeading: "Vérifie un VIN pour les rappels ouverts maintenant",
    midCtaSub: "Aperçu gratuit, instantané, sans carte de crédit. Vois les rappels de sécurité NHTSA ouverts, les résumés de défauts et la réparation gratuite chez le concessionnaire en moins de 5 secondes.",
    h2Internal: "Vérifications d'historique véhiculaire connexes",
    internalIntro: "Une vérification de rappel est un signal de sécurité. Ces vérifications couvrent les registres auxquels elle se connecte.",
    internalLinks: [
      { href: "/airbag-check", label: "Vérification airbag", desc: "Vérifie l'historique Takata et de déploiement d'airbag." },
      { href: "/vin-check", label: "Vérification VIN complète", desc: "Effectue un rapport complet d'historique véhiculaire." },
      { href: "/accident-history-check", label: "Vérification historique d'accidents", desc: "Examine les registres de collision et de dommages." },
      { href: "/warranty-check", label: "Vérification de garantie", desc: "Vois comment les rappels se rapportent à la couverture d'usine." },
      { href: "/lemon-check", label: "Vérification lemon", desc: "Trouve les registres de rachat et de défauts répétés." },
      { href: "/salvage-title-check", label: "Vérification titre salvage", desc: "Vérifie les marques de titre salvage et reconstruit." },
      { href: "/odometer-check", label: "Vérification odomètre", desc: "Recoupe le kilométrage pour la remise à zéro." },
      { href: "/vin-decoder", label: "Décodeur VIN", desc: "Décode les détails de fabrication derrière le VIN." },
    ],
    h2Faq: "Questions fréquentes — Vérification VIN des rappels",
    faqIntro: "Les questions que les conducteurs et acheteurs de voitures d'occasion posent le plus sur les rappels de sécurité.",
    bottomBadge: "Aperçu gratuit · Instantané · Données NHTSA",
    ctaBottomHeading: "Ne conduis pas avec un rappel non réparé",
    ctaBottomSub: "Un rappel ouvert peut se cacher derrière une voiture qui paraît propre. Une vérification VIN montre chaque campagne NHTSA non réparée et la réparation gratuite qui t'attend chez le concessionnaire, en 5 secondes.",
    h2Sources: "Sources et autorité des données",
    sourcesIntro: "Les données de rappels sont lues aux côtés des registres fédéraux de sécurité et de titre pour que l'image complète soit cohérente. Voici les sources principales et les agences avec lesquelles tu peux recouper.",
    sourcesNote: "La disponibilité des rappels varie selon le véhicule. Les campagnes annoncées très récemment peuvent prendre du temps à apparaître dans une recherche VIN, et un résultat propre ne remplace pas la confirmation du statut auprès d'un concessionnaire franchisé avant l'achat.",
  },
} as const;
