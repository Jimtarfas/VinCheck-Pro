/**
 * Shared body for /jeep-vin-check/[model].
 *
 ...
 * blurb, angle, check areas, tips, FAQ, how-to) from JEEP_MODELS via the
 * helpers in src/lib/jeep-models.ts. Mirrors the section structure and
...
export default function JeepVinBody({ modelSlug }: { modelSlug: string }) {
  const m = findJeepModel(modelSlug);
  if (!m) return null;

  const homeHref = "/";
  const hubHref = "/jeep-vin-check";
  const vinCheckHref = "/vin-check";

  const others = getOtherJeepModels(modelSlug, 4);
  const faqs = jeepFaqs(m);
  const howToSteps = jeepHowTo(m);
  ...
  return (
    <article className="pb-16 bg-surface">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-24 pb-14 sm:pt-28 sm:pb-20">
          <Breadcrumbs
            items={[
              ...
              { label: "Jeep VIN Check", href: hubHref },
              { label: m.name },
            ]}
            onDark
          />
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        ...
        <section className="py-12 sm:py-16 border-b border-outline-variant">
          <h2 className="text-2xl sm:text-3xl font-headline font-extrabold text-primary mb-4">
            Is the Jeep {m.name} Reliable?
          </h2>
          <div className="prose prose-slate max-w-none text-sm sm:text-base text-on-surface-variant leading-relaxed space-y-4">
            <p>
              Reliability is a{" "}
              <strong className="text-on-surface">per-vehicle question</strong>,
              not a per-model verdict. Jeep builds large volumes of
        ...
        <section className="py-12 sm:py-16 border-b border-outline-variant">
          <h2 className="text-2xl sm:text-3xl font-headline font-extrabold text-primary mb-2">
            VIN Checks for Other Jeep Models
          </h2>
          <p className="text-sm sm:text-base text-on-surface-variant mb-7">
            ...
            {" "}{m.name} with these model guides, or browse every Jeep model.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <Link href={hubHref} className="flex items-start gap-3 rounded-xl border border-outline-variant bg-surface hover:border-primary/40 hover:bg-primary/5 transition-colors p-4 group">
              ...
              <div>
                <div className="text-sm font-bold text-primary group-hover:underline">All Jeep Models</div>
                <div className="text-xs text-on-surface-variant mt-0.5">Browse every Jeep VIN guide.</div>
              </div>
            </Link>
