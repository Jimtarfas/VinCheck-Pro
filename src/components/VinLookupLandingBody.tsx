/**
 * Shared body for the /vin-lookup/[type] hub-and-spoke SEO cluster.
 *
 * Renders a single lookup landing page from a LookupPage data entry. Unlike
 * the decoder cluster (which embeds the spec-only decoder widget), this
 * cluster's job is lead generation: every page leads with the
 * <VinSearchForm /> that routes into the /report-preview conversion flow,
 * and repeats a "Run VIN Check / Get your report" CTA mid-page and at the
...
import {
  type LookupPage,
  relatedLookupPages,
} from "@/lib/vin-lookup-pages";

export default function VinLookupLandingBody({ page }: { page: LookupPage }) {
  const siblings = relatedLookupPages(page.relatedSlugs);
          ...
          <div className="rounded-3xl bg-primary p-7 sm:p-10">
            <div className="text-center mb-6">
              ...
              <p className="text-white/80 text-sm sm:text-base max-w-xl mx-auto">
                ...
                report — title brands, accidents, odometer, recalls and more.
              </p>
            </div>
