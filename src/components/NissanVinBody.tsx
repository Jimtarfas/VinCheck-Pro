/**
 * Shared body for /nissan-vin-check/[model].
 *
 ...
 * blurb, angle, check areas, tips, FAQ, how-to) from NISSAN_MODELS via the
 * helpers in src/lib/nissan-models.ts. Mirrors the section structure and
...
export default function NissanVinBody({ modelSlug }: { modelSlug: string }) {
  const m = findNissanModel(modelSlug);
  if (!m) return null;

  const homeHref = "/";
  const hubHref = "/nissan-vin-check";
  const vinCheckHref = "/vin-check";

  const others = getOtherNissanModels(modelSlug, 4);
  const faqs = nissanFaqs(m);
  const howToSteps = nissanHowTo(m);
  ...
  return (
    <article className="pb-16 bg-surface">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-24 pb-14 sm:pt-28 sm:pb-20">
          <Breadcrumbs
            items={[
              ...
              { label: "Nissan VIN Check", href: hubHref },
              { label: m.name },
            ]}
            onDark
          />
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="rounded-2xl border border-outline-variant bg-surface-container-lowest p-5">
              ...
              <ul className="space-y-2 text-sm text-on-surface-variant">
                ...
                <li className="flex gap-2"><Check className="w-4 h-4 text-primary flex-shrink-0 mt-0.5" strokeWidth={3} /><span>WMI prefix {m.vinPrefix} identifies the Nissan plant and country of assembly.</span></li>
        ...
        <section className="py-12 sm:py-16 border-b border-outline-variant">
          <h2 className="text-2xl sm:text-3xl font-headline font-extrabold text-primary mb-4">
            Is the Nissan {m.name} Reliable?
          </h2>
          <div className="prose prose-slate max-w-none text-sm sm:text-base text-on-surface-variant leading-relaxed space-y-4">
            <p>
              Reliability is a{" "}
              <strong className="text-on-surface">per-vehicle question</strong>,
              not a per-model verdict. Nissan builds large volumes of
        ...
        <section className="py-12 sm:py-16 border-b border-outline-variant">
          <h2 className="text-2xl sm:text-3xl font-headline font-extrabold text-primary mb-2">
            VIN Checks for Other Nissan Models
          </h2>
          <p className="text-sm sm:text-base text-on-surface-variant mb-7">
            ...
            {" "}{m.name} with these model guides, or browse every Nissan model.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <Link href={hubHref} className="flex items-start gap-3 rounded-xl border border-outline-variant bg-surface hover:border-primary/40 hover:bg-primary/5 transition-colors p-4 group">
              ...
              <div>
                <div className="text-sm font-bold text-primary group-hover:underline">All Nissan Models</div>
                <div className="text-xs text-on-surface-variant mt-0.5">Browse every Nissan VIN guide.</div>
              </div>
            </Link>
