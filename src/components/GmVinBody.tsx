/**
 * Shared body for /gm-vin-check/[model] (the dynamic segment is a GM DIVISION).
 *
 * Each of the 10 division spokes renders from this one component, pulling
 * its facts (lineup focus, active years, WMI prefix, VIN + RPO/SPID location,
 * what the VIN decodes, blurb, angle, check areas, tips, FAQ, how-to) from
 * GM_DIVISIONS via the helpers in src/lib/gm-models.ts. Mirrors the section
 * structure and design tokens of the Chevrolet cluster so the GM pages look
 * native to the site, but is tuned for division-level "GM VIN number decoder"
...
const TRUST_ICONS = [Car, ShieldCheck, Clock, BadgeCheck, Fingerprint] as const;

export default function GmVinBody({ modelSlug }: { modelSlug: string }) {
  const m = findGmDivision(modelSlug);
  if (!m) return null;

  const homeHref = "/";
  const hubHref = "/gm-vin-check";
  const vinCheckHref = "/vin-check";

  const others = getOtherGmDivisions(modelSlug, 4);
  const faqs = gmFaqs(m);
  const howToSteps = gmHowTo(m);

  const HEADLINE_STATS = [
    { value: m.bodyStyle, label: "Lineup focus" },
    { value: m.generation, label: "Active years" },
    { value: m.vinPrefix, label: "GM WMI prefix" },
    { value: m.segment, label: "Market position" },
  ];

  const TRUST_STATS = [
    { icon: TRUST_ICONS[0], value: m.name, label: "division-specific decode" },
  ...
  return (
    <article className="pb-16 bg-surface">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-24 pb-14 sm:pt-28 sm:pb-20">
          <Breadcrumbs
            items={[
              ...
              { label: "GM VIN Decoder", href: hubHref },
              { label: m.name },
            ]}
            onDark
          />
          ...
          <p className="speakable-intro text-base sm:text-xl text-white/85 max-w-3xl mb-8 leading-relaxed">
            Run a free {m.fullName} VIN decode to read the year, division, and
            plant, pair it with the GM RPO/SPID build codes, and reveal any
            salvage, flood, lemon-law buyback, accident, or odometer-rollback
            brand on that exact {m.name}. Instant results sourced from NMVTIS
            and every state DMV — no signup, no credit card.
          </p>

          <div className="bg-white rounded-2xl p-5 sm:p-7 shadow-xl">
            <h2 className="text-base sm:text-lg font-headline font-extrabold text-primary mb-1">
              Run a Free {m.name} VIN Decode
            </h2>
            <p className="text-xs sm:text-sm text-on-surface-variant mb-4">
              Enter the 17-character VIN from the {m.name}
            </p>
      ...
      <section aria-labelledby="division-stats-heading" className="max-w-5xl mx-auto px-4 sm:px-6 -mt-8 sm:-mt-10 relative z-10">
        <div className="rounded-3xl bg-surface-container shadow-md border border-outline-variant p-5 sm:p-7">
          <h2 id="division-stats-heading" className="text-xs sm:text-sm font-headline font-black uppercase tracking-widest text-primary mb-4 sm:mb-5">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        <section className="py-12 sm:py-16 border-b border-outline-variant">
          <h2 className="text-2xl sm:text-3xl font-headline font-extrabold text-primary mb-4">
            What a {m.name} VIN Decode Tells You
          </h2>
          <div className="prose prose-slate max-w-none text-sm sm:text-base text-on-surface-variant leading-relaxed space-y-4">
            ...
            <p>
              ...
              lemon-law buyback, or total loss — is gathered into{" "}
              ...
              by re-titling the vehicle somewhere else.
            </p>
          </div>
        </section>
        ...
        <section className="py-12 sm:py-16 border-b border-outline-variant">
          <h2 className="text-2xl sm:text-3xl font-headline font-extrabold text-primary mb-2">
            {m.name} VIN Basics — Where to Find It &amp; What It Decodes
          </h2>
          <p className="text-sm sm:text-base text-on-surface-variant mb-7 max-w-3xl">
            A 17-character VIN identifies one specific {m.name} vehicle. Here is
            where it lives and what its characters — plus the GM RPO/SPID build
            codes — reveal.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="rounded-2xl border border-outline-variant bg-surface-container-lowest p-5">
              ...
              <ul className="space-y-2 text-sm text-on-surface-variant">
                ...
                <li className="flex gap-2"><Check className="w-4 h-4 text-primary flex-shrink-0 mt-0.5" strokeWidth={3} /><span>WMI prefix {m.vinPrefix} identifies the GM division, plant, and country of assembly.</span></li>
                <li className="flex gap-2"><Check className="w-4 h-4 text-primary flex-shrink-0 mt-0.5" strokeWidth={3} /><span>Model year and trim, paired with the RPO/SPID codes, so you can confirm the listing matches the real {m.name}.</span></li>
              </ul>
            </div>
          </div>
        </section>
        ...
        <section className="py-12 sm:py-16 border-b border-outline-variant">
          <h2 className="text-2xl sm:text-3xl font-headline font-extrabold text-primary mb-2">
            What to Verify on a Used {m.name}
          </h2>
          <p className="text-sm sm:text-base text-on-surface-variant mb-7 max-w-3xl">
            Buyer-reported areas worth confirming by VIN, RPO/SPID codes, recall
            lookup, and an in-person inspection — these are things to check, not
            verdicts on the division.
          </p>
        ...
        <section className="py-12 sm:py-16 border-b border-outline-variant">
          <h2 className="text-2xl sm:text-3xl font-headline font-extrabold text-primary mb-2">
            Smart Buyer Tips for a {m.name}
          </h2>
          <p className="text-sm sm:text-base text-on-surface-variant mb-7 max-w-3xl">
            Division-specific pointers that make a problem {m.name} easier to
        ...
        <section className="py-12 sm:py-16 border-b border-outline-variant">
          <h2 className="text-2xl sm:text-3xl font-headline font-extrabold text-primary mb-2">
            How to Decode a {m.name} VIN — 6 Steps
          </h2>
        ...
        <section className="py-12 sm:py-16 border-b border-outline-variant">
          <h2 className="text-2xl sm:text-3xl font-headline font-extrabold text-primary mb-4">
            Is a GM {m.name} Reliable?
          </h2>
          <div className="prose prose-slate max-w-none text-sm sm:text-base text-on-surface-variant leading-relaxed space-y-4">
            <p>
              Reliability is a{" "}
              <strong className="text-on-surface">per-vehicle question</strong>,
              not a per-division verdict. General Motors built large volumes of
              trouble-free {m.name} vehicles, and even a model year with many
              NHTSA complaints has far more clean-running examples than problem
              ones.
            </p>
            <p>
              ...
              High complaint clusters describe a model year, not the specific
              vehicle in front of you — which is exactly why a{" "}
              <Link href={vinCheckHref} className="text-primary font-bold hover:underline">
                VIN-level history check
              </Link>{" "}
              beats brand reputation: it tells you about the one {m.name} you are
              about to buy.
            </p>
          </div>
        </section>
        ...
        <section className="py-10">
          <div className="rounded-3xl bg-primary p-7 sm:p-10 text-center">
            ...
            <h2 className="text-2xl sm:text-3xl font-headline font-extrabold text-white mb-2">
              Don&apos;t Buy a Branded {m.name} by Mistake
            </h2>
            <p className="text-white/80 text-sm sm:text-base mb-6 max-w-xl mx-auto">
              Free, instant {m.name} VIN decode sourced from NMVTIS and every
        ...
        <section className="py-12 sm:py-16 border-b border-outline-variant">
          ...
          <p className="text-sm sm:text-base text-on-surface-variant mb-7">
            VIN locations, WMI prefixes, and RPO codes differ by division.
            Compare {m.name} with these GM marque guides, or browse every GM
            division.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {others.map((o) => (
              ...
              >
                ...
                <div>
                  <div className="text-sm font-bold text-primary group-hover:underline">{o.name} VIN Decoder</div>
            ...
            <Link href={hubHref} className="flex items-start gap-3 rounded-xl border border-outline-variant bg-surface hover:border-primary/40 hover:bg-primary/5 transition-colors p-4 group">
              ...
              <div>
                <div className="text-sm font-bold text-primary group-hover:underline">All GM Divisions</div>
                <div className="text-xs text-on-surface-variant mt-0.5">Browse every GM division VIN guide.</div>
              </div>
            </Link>
        ...
        <section className="py-12 sm:py-16 border-b border-outline-variant">
          <h2 className="text-2xl sm:text-3xl font-headline font-extrabold text-primary mb-2">
            {m.fullName} VIN Decoder FAQ
          </h2>
          <p className="text-sm text-on-surface-variant mb-8">
            The most-searched questions about decoding and checking a {m.name} VIN.
          </p>
        ...
        <section className="py-14 sm:py-20 text-center">
          ...
          <h2 className="text-2xl sm:text-4xl font-headline font-extrabold text-primary mb-3">
            One VIN. Your {m.name}&apos;s Full History. Five Seconds.
          </h2>
          <p className="text-sm sm:text-base text-on-surface-variant max-w-2xl mx-auto mb-8">
            A salvage, flood, or buyback record follows the VIN permanently, even
            when the paper title looks clean. Run the free {m.name} decode before
            you write a check.
          </p>
