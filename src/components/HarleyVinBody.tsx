/**
 * Shared body for /harley-davidson-vin-check/[model].
 *
 * Each of the 10 model spokes renders from this one component, pulling
 * its facts (motorcycle style, generation, WMI prefix, VIN location,
 * engine family, blurb, angle, check areas, tips, FAQ, how-to) from
 * HARLEY_MODELS via the helpers in src/lib/harley-models.ts. Mirrors the
 * section structure and design tokens of LemonCheckBrandBody so the
 * cluster looks native to the site, but is tuned for model-specific
...
import {
  findHarleyModel,
  getOtherHarleyModels,
  harleyFaqs,
  harleyHowTo,
} from "@/lib/harley-models";
...
export default function HarleyVinBody({ modelSlug }: { modelSlug: string }) {
  const m = findHarleyModel(modelSlug);
  if (!m) return null;

  const homeHref = "/";
  const hubHref = "/harley-davidson-vin-check";
  const vinCheckHref = "/vin-check";

  const others = getOtherHarleyModels(modelSlug, 4);
  const faqs = harleyFaqs(m);
  const howToSteps = harleyHowTo(m);

  const HEADLINE_STATS = [
    { value: m.bodyStyle, label: "Motorcycle style" },
    { value: m.generation, label: "Generation window" },
    { value: m.vinPrefix, label: "WMI prefix" },
    { value: m.segment, label: "Class" },
  ];
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-24 pb-14 sm:pt-28 sm:pb-20">
          <Breadcrumbs
            items={[
              ...
              { label: "Harley-Davidson VIN Check", href: hubHref },
              { label: m.name },
            ]}
            onDark
          />
          ...
          <p className="speakable-intro text-base sm:text-xl text-white/85 max-w-3xl mb-8 leading-relaxed">
            ...
            odometer-rollback brand on that exact {m.name}. Instant results
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        <section className="py-12 sm:py-16 border-b border-outline-variant">
          ...
          <div className="prose prose-slate max-w-none text-sm sm:text-base text-on-surface-variant leading-relaxed space-y-4">
            ...
            <p>
              ...
              by re-titling the motorcycle somewhere else.
            </p>
          </div>
        </section>
        ...
        <section className="py-12 sm:py-16 border-b border-outline-variant">
          ...
          <p className="text-sm sm:text-base text-on-surface-variant mb-7 max-w-3xl">
            ...
            lives on this motorcycle and what its characters reveal.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="rounded-2xl border border-outline-variant bg-surface-container-lowest p-5">
              ...
              <ul className="space-y-2 text-sm text-on-surface-variant">
                ...
                <li className="flex gap-2"><Check className="w-4 h-4 text-primary flex-shrink-0 mt-0.5" strokeWidth={3} /><span>WMI prefix {m.vinPrefix} identifies Harley-Davidson as the manufacturer.</span></li>
                <li className="flex gap-2"><Check className="w-4 h-4 text-primary flex-shrink-0 mt-0.5" strokeWidth={3} /><span>Model year and model designation, so you can confirm the listing matches the real {m.name}.</span></li>
              </ul>
            </div>
          </div>
        </section>
        ...
        <section className="py-12 sm:py-16 border-b border-outline-variant">
          <h2 className="text-2xl sm:text-3xl font-headline font-extrabold text-primary mb-4">
            Is the Harley-Davidson {m.name} Reliable?
          </h2>
          <div className="prose prose-slate max-w-none text-sm sm:text-base text-on-surface-variant leading-relaxed space-y-4">
            <p>
              Reliability is a{" "}
              <strong className="text-on-surface">per-motorcycle question</strong>,
              not a per-model verdict. Harley-Davidson builds large volumes of
            ...
            <p>
              ...
              High complaint clusters describe a model year, not the specific bike
        ...
        <section className="py-12 sm:py-16 border-b border-outline-variant">
          <h2 className="text-2xl sm:text-3xl font-headline font-extrabold text-primary mb-2">
            VIN Checks for Other Harley-Davidson Models
          </h2>
          <p className="text-sm sm:text-base text-on-surface-variant mb-7">
            VIN locations and decode details differ by motorcycle style. Compare the
            {" "}{m.name} with these model guides, or browse every Harley-Davidson model.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <Link href={hubHref} className="flex items-start gap-3 rounded-xl border border-outline-variant bg-surface hover:border-primary/40 hover:bg-primary/5 transition-colors p-4 group">
              ...
              <div>
                <div className="text-sm font-bold text-primary group-hover:underline">All Harley-Davidson Models</div>
                <div className="text-xs text-on-surface-variant mt-0.5">Browse every Harley-Davidson VIN guide.</div>
              </div>
            </Link>
