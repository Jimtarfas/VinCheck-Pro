...
const COPY = {
  en: {
    ...
    intro: "CarCheckerVIN's free dealer history check queries NMVTIS, all 50 state DMVs, and manufacturer in-service and dealer-trade records to surface demonstrator (demo), service-loaner, press-fleet, and dealer-trade use for any 17-character VIN. As an NMVTIS-approved data provider, CarCheckerVIN returns the title chain and in-service date that reveal whether the car spent its first miles on a dealer lot — directly affecting the warranty clock and the price you should pay. Not every new or nearly-new car was bought fresh off the truck by a consumer. Dealers keep vehicles as demonstrators (demo cars), lend them as service loaners, and sometimes assign them to press fleets or manufacturer representatives — all before the vehicle is ever \"sold.\" A VIN dealer history check reveals whether a vehicle accumulated mileage in dealer hands before you bought it, which directly affects its warranty clock, real-world value, and condition.",
