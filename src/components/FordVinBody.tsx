/**
 * Shared body for /ford-vin-check/[model].
 *
 ...
 * blurb, angle, check areas, tips, FAQ, how-to) from FORD_MODELS via the
 * helpers in src/lib/ford-models.ts. Mirrors the section structure and
...
export default function FordVinBody({ modelSlug }: { modelSlug: string }) {
  const m = findFordModel(modelSlug);
  if (!m) return null;

  const homeHref = "/";
  const hubHref = "/ford-vin-check";
  const vinCheckHref = "/vin-check";

  const others = getOtherFordModels(modelSlug, 4);
  const faqs = fordFaqs(m);
  const howToSteps = fordHowTo(m);

  const HEADLINE_STATS = [
    { value: m.bodyStyle, label: "Body style" },
    { value: m.generation, label: "Generation window" },
    { value: m.vinPrefix, label: "Ford WMI prefix" },
  ...
  return (
    <article className="pb-16 bg-surface">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-24 pb-14 sm:pt-28 sm:pb-20">
          <Breadcrumbs
            items={[
              ...
              { label: "Ford VIN Check", href: hubHref },
              { label: m.name },
            ]}
            onDark
          />
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="rounded-2xl border border-outline-variant bg-surface-container-lowest p-5">
              ...
              <ul className="space-y-2 text-sm text-on-surface-variant">
                ...
                <li className="flex gap-2"><Check className="w-4 h-4 text-primary flex-shrink-0 mt-0.5" strokeWidth={3} /><span>WMI prefix {m.vinPrefix} identifies the Ford plant and country of assembly.</span></li>
        ...
        <section className="py-12 sm:py-16 border-b border-outline-variant">
          <h2 className="text-2xl sm:text-3xl font-headline font-extrabold text-primary mb-4">
            Is the Ford {m.name} Reliable?
          </h2>
          <div className="prose prose-slate max-w-none text-sm sm:text-base text-on-surface-variant leading-relaxed space-y-4">
            <p>
              Reliability is a{" "}
              <strong className="text-on-surface">per-vehicle question</strong>,
              not a per-model verdict. Ford builds large volumes of
        ...
        <section className="py-12 sm:py-16 border-b border-outline-variant">
          ...
          <p className="text-sm sm:text-base text-on-surface-variant mb-7">
            ...
            {" "}{m.name} with these model guides, or browse every Ford model.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <Link href={hubHref} className="flex items-start gap-3 rounded-xl border border-outline-variant bg-surface hover:border-primary/40 hover:bg-primary/5 transition-colors p-4 group">
              ...
              <div>
                <div className="text-sm font-bold text-primary group-hover:underline">All Ford Models</div>
                <div className="text-xs text-on-surface-variant mt-0.5">Browse every Ford VIN guide.</div>
              </div>
            </Link>
